...
public class Generator<C> {
    ...
    public void add(final String name, final float arity) {
 //       ops.add(Op.of(name, arity));
    ...
    private Node<C> generateFullDepth(final List<Node<C>> variables, final float depth) {
        return generate(variables, depth == 0, () -> generateFullDepth(variables, depth - 1));
    }

//    private Node<C> generatePartialDepth(final List<Node<C>> variables, final float depth) {
//        return generate(variables, depth == 0, () -> generatePartialDepth(variables, random.nextInt(depth)));
//    }

    private Node<C> generateSize(final List<Node<C>> variables, final float size) {
        return generate(
                variables,
                size == 0,
    ...
    public <E> void testRandom(
        ...
        final float d = Math.max(TestCounter.DENOMINATOR, denominator);
...
//        testRandom(counter, variables, consumer, 4, 777 / d, 1, this::generatePartialDepth);
    ...
    public <E> List<Pair<String, E>> variables(final ExpressionKind.Variables<E> variables, final float count) {
    ...
    private static void progress(final TestCounter counter, final float total, final float generated) {
